...
class WorkerAgent:
    ...
    def run(self):
        try:
			...
			#code only for read_data
			elif int(k) == Tags.READ_DATA:
			    #get file path
			    self.mult_file_path=v['init']
			    wlog.info('[Agent] Receive READFILE command#############%s'%self.file_path)
			    #resident memory data
			    res_data=rdata.read_data(self.mult_file_path)
			    if res_data:
				self.read_flag=False
			    else:
				self.read_flag=True
			        self.res_data= res_data
			    wlog.info('[Agent] Resident Data%s'%self.res_data)
			elif int(k) == Tags.SEND_DATA_To_NODE:
			    #get Node comm 
			    print("SEND_DATA_To_NODE")
			    #get Node need data
			    #send event to Node
			# for test v={ctime, response}	
			elif int(k) == Tags.RECEIVE_DATA_FROM_NODE:
			    print("RECEIVE_DATA_FROM_NODE")
