...
script_dict={
    ...
    'read_data': "/root/mpi/ProdTest/res/Read/data_1/e+_1.0Mev/read.sh"
}
...
class AnaApp(JunoApp):
    def __init__(self,rootdir, name, config_path=None):
        if not config_path or not os.path.exists(os.path.abspath(config_path)):
            config_path='/root/mpi/server/Application/AnalysisApp/config.ini'
    ...
    def _find_data(self):
        ...
        for sample in sample_list:
            ...
            for subdir in os.listdir(self.data_path+'/'+sample):  #/home/liuyan/mpi/ProdTest/res/  + Read
                subdir=self.data_path+'/'+sample+'/'+subdir       #/home/liuyan/mpi/ProdTest/res/  + Read + data_1
                if os.path.isdir(subdir):
                    #sample_dict[os.path.basename(sample)][os.path.subdir]={}
                    for energy in os.listdir(subdir):             #/home/liuyan/mpi/ProdTest/res/  + Read + data_1 + e+_1.0Mev
    ...
    def _gen_list(self, energy_path):
        ...
        for step in self.workflow:
            if not os.path.exists(energy_path+'/'+step):
                os.mkdir(energy_path+'/'+step)
    ...
    def split(self):
        ...
        for step in self.workflow:
            print("workflow is: "+self.workflow[0])
            print(energy_path)
            for energy in energy_path:
                if step in os.listdir(energy):
                    ...
                    if step == 'read_data':
                        ...
                        print("task is : "+step+" "+self.step_script[step])
...
if __name__ == "__main__":
    app = AnaApp(os.environ['DistJETPATH_Server']+"/Application/AnalysisApp/", 'AnalysisApp',"/root/mpi/server/Application/AnalysisApp/config.ini")
    app.res_dir = (os.environ['DistJETPATH_Server']+'/Application/AnalysisApp/test')
    task_list = app.split()
